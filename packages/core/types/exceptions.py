...
class KnownException(tum_esm_utils.validators.StricterBaseModel):
    ...
    identifier: str = pydantic.Field(
        ..., description="Stable identifier used to persist this exception type."
    )
...
class KNOWN_EXCEPTIONS:
    ...
    COVER_DID_NOT_CLOSE = KnownException(
        identifier="cover-did-not-close",
    ...
    TUM_ENCLOSURE_PLC_CONNECTION_ERROR = KnownException(
        identifier="tum-enclosure-plc-connection-error",
    ...
    TUM_ENCLOSURE_RAIN_DETECTED_COVER_NOT_CLOSED = KnownException(
        identifier="tum-enclosure-rain-detected-cover-not-closed",
    ...
    TUM_ENCLOSURE_PLC_RESET_FAILED = KnownException(
        identifier="tum-enclosure-plc-reset-failed",
    ...
    TUM_ENCLOSURE_PLC_ERROR = KnownException(
        identifier="tum-enclosure-plc-error",
    ...
    AEMET_ENCLOSURE_DATALOGGER_ERROR = KnownException(
        identifier="aemet-enclosure-datalogger-error",
    ...
    CAMTRACKER_CONFIGURATION_ERROR = KnownException(
        identifier="camtracker-configuration-error",
    ...
    HELIOS_CAMERA_ERROR = KnownException(
        identifier="helios-camera-error",
    ...
    OPUS_CONNECTION_ERROR = KnownException(
        identifier="opus-connection-error",
    ...
    EM27_CONNECTION_ERROR = KnownException(
        identifier="em27-connection-error",
    ...
    UNEXPECTED_ERROR = KnownException(
        identifier="unexpected-error",
        error_message="An unexpected error occurred.",
        long_description=(
            "PYRA encountered an error that could not be assigned to a more specific sdocumented exception type. Refer to the traceback for the underlying cause."
        ),
    )

    @classmethod
    def all(cls) -> tuple[KnownException, ...]:
        """Return all registered exception catalog entries."""

        return tuple(value for value in vars(cls).values() if isinstance(value, KnownException))

    @classmethod
    def is_registered(cls, exception_type: KnownException) -> bool:
        """Return whether an object is a registered catalog entry."""

        return any(exception_type is entry for entry in cls.all())
