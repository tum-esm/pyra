...
The config file under `config/config.json` contains all parameters to tweak Pyra's operation. Schema: [/packages/core/types/config.py](https://github.com/tum-esm/pyra/blob/main/packages/core/types/config.py).
...
_Pyra Core_ stores its activity (is it measuring, are there errors, etc.) in `logs/activity/activity-YYYY-MM-DD.json`. This is the same information as in the regular log files but significantly easier to parse. Schema: [/packages/core/types/activity_history.py](https://github.com/tum-esm/pyra/blob/main/packages/core/types/activity_history.py).
