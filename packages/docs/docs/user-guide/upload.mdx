...
The idea of the `complete=true` flag is that the postprocessing should only consider data where the upload is complete. This completeness cannot be guaranteed by a mandatory time delay because when the measurement computers don't have internet for a while, they could upload it after that delay threshold.
