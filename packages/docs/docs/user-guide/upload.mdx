...
When using [Helios](/docs/user-guide/tum-plc-and-helios), saved images will be stored inside `/logs/helios/` in the same directory structure as the interferogram output directory. This is why the upload can be used for both types of data.
...
The idea of the `complete=true` flag is that the postprocessing should only consider data where the upload is complete.
