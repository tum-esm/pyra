...
Pyra's measurement mode can be selected in the overview tab. In the logs and the codebase, this system of determining whether to measure or not is called **Condition Assessment System (CAS)**.
...
CLI mode can be used when you have already built a system that evaluates measurement conditions and want to tell Pyra when to start and stop measurements from an external source. Respective CLI commands are in the [next section](#cli-command-line-interface).
