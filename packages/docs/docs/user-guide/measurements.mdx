...
The usage of any command is explained in its help menu (available in command line).
...
Pyra will validate the structure of the update and enforce some rules on the parameter values (value range, file existance, etc.)
...
### CLI: Starting and stopping measurements via CLI
