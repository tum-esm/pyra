...
The following manual steps are only required at initial system setup. All future updates and new installations of Pyra can be done with the [Pyra Setup Tool](#pyra-setup-tool).
...
1. Download each releases codebase
...
5. (Optional) Migrate the `config.json` file to keep previous settings
