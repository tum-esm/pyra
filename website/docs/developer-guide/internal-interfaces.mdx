...
The config file under `config/config.json` contains all parameters to tweak PYRA's operation. Its schema:
...
The state-file is generated under `runtime-data/state.json`. PYRA Core writes its internal values to this files. The state-file is used to communicate between modules as well as with the "outside" world (UI, CLI). Its schema:
...
The persistent-state-file is generated under `logs/persistent-state.json`. It is currently only used by the Core and will not be resetted with a restart of the Core. Its schema:
...
MyPy will make full use of these type schemas (see [testing](/docs/developer-guide/testing-and-ci)). The full config-validation (schema + value rules) runs at the beginning of every PYRA Core loop iteration. Additionally, whenever loading the state-files, the respective schema validation will run. Hence, PYRA will detect when a state file does not have the expected schema and raise an Exception. All internal code interfaces (function calls, etc.) are covered by the strict MyPy validation.
...
The UI reads the log files and the state file periodically using [Tauri's filesystem API](https://tauri.app/v1/api/js/modules/fs). We tried using sockets or filewatchers, but both did not work well on windows and reading it periodically is the most simplistic implementation.
