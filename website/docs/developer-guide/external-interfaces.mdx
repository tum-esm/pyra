...
We are using the `pywin32` Python library which provides an interface to the [Windows DDE APIs](https://en.wikipedia.org/wiki/Dynamic_Data_Exchange). Pyra Core uses DDE to communicate with OPUS: Loading experiment files, starting and stopping macros, stopping OPUS itself.
...
The `LEARN_Az_Elev.dat` file contains CamTracker's logs about mirror position and currently estimated sun position. Pyra Core Example:
