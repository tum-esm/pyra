...
def test_version_numbers() -> None:
    ...
    cli_info_stdout = tum_esm_utils.shell.run_shell_command(
        f"python ./packages/cli/main.py info",
        working_directory=PROJECT_DIR,
        executable=None,
    ).strip(" \n")
    assert (
        cli_info_stdout
        == f'This CLI is running Pyra version {pyproject_version} in directory "{PROJECT_DIR}"'
    )
    ...
    with open(os.path.join(PROJECT_DIR, "packages", "ui", "package.json"), "r") as f:
        ui_package_version = json.load(f)["version"]
    assert ui_package_version == pyproject_version

    with open(os.path.join(PROJECT_DIR, "packages", "ui", "src-tauri", "Cargo.toml"), "r") as f:
        third_line = f.read().split("\n")[2]
        assert third_line.startswith("version = ")
