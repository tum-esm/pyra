...
def test_start_stop_procedure(original_config, original_logs):
    ...
    expected_lines = [
        "pyra.core.main - INFO - Starting Iteration",
        "pyra.core.main - INFO - pyra-core in test mode",
        "pyra.core.main - INFO - Ending Iteration",
    ]
    now = datetime.utcnow()
    ...
    assert stdout_6.startswith(
        f"Terminated 1 background processe(s) with PID(s) [{pid}]"
    )
